...
- (void)setMessage:(WeiboMessage *)message {
    ...
    CGFloat created_at_Y = CGRectGetMaxY(_screen_name_Lbl_frame) + magin;
    ...
    //正文
    CGFloat textX = magin;
    CGFloat textY = CGRectGetMaxX(_user_Img_frame) + magin;
    ...
    _attitudes_count_Lbl_frame = CGRectMake(attitudeLblX, lblY, lblW, lblH);
    
    //转发数
    CGFloat repostX = 1 *lblW;

    _reposts_count_Lbl_frame = CGRectMake(repostX, lblY, lblW, lblH);
    
    //评论数
    CGFloat commentX = 2 * lblW;
    _comments_count_Lbl_frame = CGRectMake(commentX, lblY, lblW, lblH);

    _rowHeight = CGRectGetMaxY(_comments_count_Lbl_frame);
}
